...
local function applyPanelStyle()
	if panel then
		panel.BackgroundTransparency = state.panelTransparency
	end
end
...
local function startSpinner()
	if state.spinnerConnection then
		state.spinnerConnection:Disconnect()
	end
	state.spinnerConnection = RunService.RenderStepped:Connect(function(delta)
		if spinner and spinner.Visible then
			spinner.Rotation = (spinner.Rotation + delta * 360) % 360
		end
	end)
end
...
local function applyWatermark()
	...
	if watermarkColorPreview then
		watermarkColorPreview.BackgroundColor3 = state.watermarkColor
	end
...
local function applyBranding()
	...
	if logo == "" or string.lower(logo) == "sparkle" then
		state.logoMode = "sparkle"
	else
		state.logoMode = "text"
	end
	if title == "" then
	...
	if logoFrame then
		if state.logoMode == "sparkle" then
			logoTextLabel = nil
			drawSparkle(logoFrame)
		else
			clearChildren(logoFrame)
			logoTextLabel = create("TextLabel", {
				Size = UDim2.fromScale(1, 1),
				BackgroundTransparency = 1,
				Text = state.logoText,
				TextColor3 = state.accent,
				TextScaled = true,
				Font = Enum.Font.GothamSemibold,
				Parent = logoFrame
			})
...
local function applyAccent()
	if accentPreview then
		accentPreview.BackgroundColor3 = state.accent
	end
...
-- Get the best target for aimbot
local function getBestTarget()
	...
	for _, target in ipairs(Players:GetPlayers()) do
		...
		-- Wall check
		if state.aimWallCheck and not isVisible(targetPart) then continue end
...
local function updateAim()
	...
	if aimFOVCircle then
		aimFOVCircle.Visible = state.aimEnabled and state.aimShowFOV
		if state.aimShowFOV then
			local size = state.aimFOV * 2
			aimFOVCircle.Size = UDim2.fromOffset(size, size)
		end
	end
	...
	state.aimConnection = RunService.RenderStepped:Connect(function()
		...
		-- Check if we should aim (key requirement)
		local shouldAim = true
		if state.aimRequireKey then
			shouldAim = UserInputService:IsKeyDown(state.aimKey)
		end
		...
		if target then
			-- Highlight target
			aimTargetHighlight = Instance.new("Highlight")
			aimTargetHighlight.FillTransparency = 1
			aimTargetHighlight.OutlineColor = state.accent
			aimTargetHighlight.OutlineTransparency = 0
			aimTargetHighlight.Parent = target.part
			
			-- Move mouse towards target
			local screenCenter = camera.ViewportSize / 2
			local delta = target.screenPosition - screenCenter
			
			-- Smooth movement
			mousemoverel(delta.X * (1 - state.aimSmoothness), delta.Y * (1 - state.aimSmoothness))
		end
	end)
end
...
local function updateFOVCircle()
	if not aimFOVCircle then
		...
		corner(circle, state.aimFOV)
		stroke(circle, state.aimFOVColor, 0)
		circle.UIStroke.Thickness = 1.5
	else
		aimFOVCircle.Visible = state.aimEnabled and state.aimShowFOV
		local size = state.aimFOV * 2
		aimFOVCircle.Size = UDim2.fromOffset(size, size)
		local circle = aimFOVCircle:FindFirstChildWhichIsA("Frame")
		if circle then
			corner(circle, state.aimFOV)
			circle.UIStroke.Color = state.aimFOVColor
		end
	end
end

-- Clean up ESP for a specific player immediately
...
local function updateESP()
	...
	espContainer = create("ScreenGui", {
		Name = "ESPContainer",
		ResetOnSpawn = false,
		Parent = playerGui
	})
	
	-- Track when players leave or die
	local function onPlayerRemoving(target)
		cleanupESPForPlayer(target.UserId)
	end
	
	-- Connect to all current players
	for _, target in ipairs(Players:GetPlayers()) do
		if target ~= player then
			-- We'll handle this in the render loop instead
		end
	end
	...
	espUpdateConnection = RunService.RenderStepped:Connect(function()
		...
		-- Clean up ESP for players who left the game
		for userId, playerESP in pairs(espObjects) do
			local foundPlayer = Players:GetPlayerByUserId(userId)
			if not foundPlayer then
				cleanupESPForPlayer(userId)
			end
		end
		...
		for _, target in ipairs(Players:GetPlayers()) do
			...
			if not character then
				-- Player died or hasn't spawned, remove their ESP immediately
				cleanupESPForPlayer(target.UserId)
				continue
			end
			...
			-- If player is dead, remove ESP immediately
			...
			if not onScreen then
				-- Hide ESP when off screen but don't destroy
				if espObjects[target.UserId] then
					for _, obj in pairs(espObjects[target.UserId]) do
			...
			-- Initialize ESP objects for this player if needed
			if not espObjects[target.UserId] then
				espObjects[target.UserId] = {}
			end
			...
			if state.espBoxes then
				...
				if not box or not box.Parent then
					...
					create("Frame", {
						...
						BackgroundColor3 = state.espColor,
						BorderSizePixel = 0,
						Parent = box
					})
					create("Frame", {
				...
				local topPosition = camera:WorldToViewportPoint(rootPart.Position + Vector3.new(0, height, 0))
				...
				box.Position = UDim2.fromOffset(position.X - boxWidth / 2, topPosition.Y)
				box.Size = UDim2.fromOffset(boxWidth, boxHeight)
				box.Visible = true
			elseif espObjects[target.UserId] and espObjects[target.UserId].box then
				espObjects[target.UserId].box.Visible = false
			end
			
			-- Name ESP
			if state.espNames then
				local nameLabel = espObjects[target.UserId] and espObjects[target.UserId].nameLabel
				if not nameLabel or not nameLabel.Parent then
					nameLabel = create("TextLabel", {
						BackgroundTransparency = 1,
						TextColor3 = state.espColor,
						Font = Enum.Font.GothamSemibold,
						TextSize = 14,
			...
			elseif espObjects[target.UserId] and espObjects[target.UserId].nameLabel then
				espObjects[target.UserId].nameLabel.Visible = false
			end
			
			-- Distance ESP
			if state.espDistance then
				local distLabel = espObjects[target.UserId] and espObjects[target.UserId].distLabel
				if not distLabel or not distLabel.Parent then
					distLabel = create("TextLabel", {
						BackgroundTransparency = 1,
						TextColor3 = state.espColor,
						Font = Enum.Font.Gotham,
						TextSize = 12,
			...
			elseif espObjects[target.UserId] and espObjects[target.UserId].distLabel then
				espObjects[target.UserId].distLabel.Visible = false
			end
			
			-- Health ESP
			if state.espHealth and humanoid.Health > 0 then
				local healthBar = espObjects[target.UserId] and espObjects[target.UserId].healthBar
				local healthFill = espObjects[target.UserId] and espObjects[target.UserId].healthFill
				...
				healthBar.Position = UDim2.fromOffset(position.X - 30, topPosition.Y)
				healthBar.Size = UDim2.fromOffset(4, 100)
				healthFill.Size = UDim2.new(1, 0, humanoid.Health / humanoid.MaxHealth, 0)
				healthBar.Visible = true
			elseif espObjects[target.UserId] and espObjects[target.UserId].healthBar then
				espObjects[target.UserId].healthBar.Visible = false
			end
			
			-- Tracers
			if state.espTracers then
				local tracer = espObjects[target.UserId] and espObjects[target.UserId].tracer
				...
				local screenCenter = camera.ViewportSize / 2
				local startPos = UDim2.fromOffset(screenCenter.X, screenCenter.Y)
				local endPos = UDim2.fromOffset(position.X, position.Y)
				
				local dx = endPos.X.Offset - startPos.X.Offset
				local dy = endPos.Y.Offset - startPos.Y.Offset
				...
				tracer.Visible = true
			elseif espObjects[target.UserId] and espObjects[target.UserId].tracer then
				espObjects[target.UserId].tracer.Visible = false
			end
		end
	end)
end
...
local function textInput(name, placeholder, parent)
	...
	local input = create("TextBox", {
		...
		Text = "",
		TextColor3 = theme.text,
		PlaceholderColor3 = Color3.fromRGB(116, 116, 126),
		TextXAlignment = Enum.TextXAlignment.Left,
		Font = Enum.Font.Gotham,
		TextSize = 14,
		Parent = shell
	...
	input.Focused:Connect(function()
		shell.UIStroke.Color = state.accent
		shell.UIStroke.Transparency = 0
	end)

	input.FocusLost:Connect(function()
		shell.UIStroke.Color = theme.stroke
		shell.UIStroke.Transparency = 0.35
	end)
...
local function settingsButton(label, parent)
	...
	button.MouseLeave:Connect(function()
		tween(button, 0.15, { BackgroundColor3 = theme.card2 })
	end)
	return button
end
...
local function sectionHeader(title, detail, parent)
	local section = create("Frame", {
		Size = UDim2.new(1, -14, 0, detail and 54 or 32),
		BackgroundTransparency = 1,
		Parent = parent
	})
...
local function renderVisuals()
	...
	-- ESP Color
	local espColorRow = create("Frame", {
	...
	for index, color in ipairs(espColors) do
		local swatch = create("TextButton", {
			Position = UDim2.fromOffset(60 + ((index - 1) * 42), 42),
			Size = UDim2.fromOffset(34, 34),
			BackgroundColor3 = color,
			AutoButtonColor = false,
			Text = "",
	...
	-- ESP Features (checkboxes in a compact layout)
	local espFeaturesRow = create("Frame", {
		Size = UDim2.new(1, -14, 0, 180),
		BackgroundColor3 = theme.card,
		BorderSizePixel = 0,
		Parent = contentFrame
	})
	...
	create("TextLabel", {
		...
		Font = Enum.Font.GothamSemibold,
		TextSize = 14,
		Parent = espFeaturesRow
	})

	local function espCheckbox(text, key, yPos)
		local container = create("Frame", {
			Position = UDim2.fromOffset(14, yPos),
		...
		local button = create("TextButton", {
			...
			BackgroundColor3 = state[key] and state.accent or theme.card2,
			AutoButtonColor = false,
			Text = state[key] and "ON" or "OFF",
			TextColor3 = state[key] and Color3.fromRGB(8, 8, 11) or theme.text,
			Font = Enum.Font.GothamSemibold,
			TextSize = 10,
			Parent = container
		})
	...
	-- Team Check
	local teamCheckRow = create("Frame", {
	...
	local teamCheckButton = create("TextButton", {
		Position = UDim2.fromOffset(14, 10),
		Size = UDim2.new(1, -28, 0, 28),
		BackgroundColor3 = state.espTeamCheck and state.accent or theme.card2,
		AutoButtonColor = false,
		Text = "Team Check: " .. (state.espTeamCheck and "ON" or "OFF"),
		TextColor3 = state.espTeamCheck and Color3.fromRGB(8, 8, 11) or theme.text,
		Font = Enum.Font.GothamSemibold,
		TextSize = 12,
		Parent = teamCheckRow
	})
...
local function renderAim()
	...
	sectionHeader("aimbot", "Enable aimbot that drags your cursor to the selected body part.", contentFrame)
	...
	-- Require Key toggle
	local requireKeyRow = create("Frame", {
		Size = UDim2.new(1, -14, 0, 48),
		BackgroundColor3 = theme.card,
		BorderSizePixel = 0,
		Parent = contentFrame
	...
	local requireKeyButton = create("TextButton", {
		Position = UDim2.fromOffset(14, 10),
		Size = UDim2.new(1, -28, 0, 28),
		BackgroundColor3 = state.aimRequireKey and state.accent or theme.card2,
		AutoButtonColor = false,
		Text = "Require Key: " .. (state.aimRequireKey and "ON" or "OFF"),
		TextColor3 = state.aimRequireKey and Color3.fromRGB(8, 8, 11) or theme.text,
		Font = Enum.Font.GothamSemibold,
		TextSize = 12,
		Parent = requireKeyRow
	})
	...
	-- Aim Key
	local aimKeyInput = labeledInput("aim key", "KeyCode (e.g., E, MouseButton2)", contentFrame)
	aimKeyInput.Text = tostring(state.aimKey.Name)
	
	-- Target Part Selection
	local partRow = create("Frame", {
		Size = UDim2.new(1, -14, 0, 180),
		BackgroundColor3 = theme.card,
		BorderSizePixel = 0,
		Parent = contentFrame
	...
	for index, part in ipairs(aimPartOptions) do
		...
		local button = create("TextButton", {
			...
			BackgroundColor3 = state.aimPart == part and state.accent or theme.card2,
			AutoButtonColor = false,
			Text = part,
			TextColor3 = state.aimPart == part and Color3.fromRGB(8, 8, 11) or theme.text,
			Font = Enum.Font.GothamSemibold,
			TextSize = 10,
			Parent = partRow
		})
	...
	-- Smoothness
	local smoothnessInput = labeledInput("smoothness", "Smoothness (0.1-1.0)", contentFrame)
	smoothnessInput.Text = tostring(state.aimSmoothness)
	
	-- FOV
	local aimFOVInput = labeledInput("aim fov", "FOV size (10-360)", contentFrame)
	aimFOVInput.Text = tostring(state.aimFOV)
	
	-- Options
	local optionsRow = create("Frame", {
	...
	applyAim.MouseButton1Click:Connect(function()
		...
		if not keyCode then
			...
			local success, result = pcall(function()
				return Enum.UserInputType[keyString]
			end)
			if success and result then
				-- For mouse buttons, we'll use a different approach
				keyCode = Enum.KeyCode.E -- Default fallback
			else
		...
		state.aimSmoothness = math.clamp(smooth or state.aimSmoothness, 0.1, 1.0)
		
		-- Parse FOV
		local fov = tonumber(aimFOVInput.Text)
		state.aimFOV = math.clamp(fov or state.aimFOV, 10, 360)
...
local function renderSettings()
	...
	local watermarkColorRow = create("Frame", {
		Size = UDim2.new(1, -14, 0, 92),
		BackgroundColor3 = theme.card,
		BorderSizePixel = 0,
		Parent = contentFrame
	...
	create("TextLabel", {
		...
		Font = Enum.Font.GothamSemibold,
		TextSize = 14,
		Parent = watermarkColorRow
	})

	watermarkColorPreview = create("Frame", {
		Position = UDim2.fromOffset(14, 42),
		Size = UDim2.fromOffset(34, 34),
		BackgroundColor3 = state.watermarkColor,
		BorderSizePixel = 0,
		Parent = watermarkColorRow
	})
	...
	for index, fontChoice in ipairs(watermarkFonts) do
		...
		local button = create("TextButton", {
			Position = UDim2.new(column / 3, 14 + (column * 4), 0, 40 + (row * 34)),
			Size = UDim2.new(1 / 3, -20, 0, 28),
			BackgroundColor3 = state.watermarkFont == fontChoice.value and state.accent or theme.card2,
			AutoButtonColor = false,
			Text = fontChoice.label,
			TextColor3 = state.watermarkFont == fontChoice.value and Color3.fromRGB(8, 8, 11) or theme.text,
		...
		stroke(button, theme.stroke, 0.6)
		button.MouseButton1Click:Connect(function()
			state.watermarkFont = fontChoice.value
			applyWatermark()
			renderSettings()
		end)
	end
	...
	local crosshairShapeRow = create("Frame", {
		Size = UDim2.new(1, -14, 0, 92),
		BackgroundColor3 = theme.card,
		BorderSizePixel = 0,
		Parent = contentFrame
	...
	create("TextLabel", {
		...
		Font = Enum.Font.GothamSemibold,
		TextSize = 14,
		Parent = crosshairShapeRow
	})

	local shapes = { "cross", "dot", "circle" }
	for index, shape in ipairs(shapes) do
		local button = create("TextButton", {
			Position = UDim2.new((index - 1) / 3, 14 + ((index - 1) * 4), 0, 42),
			Size = UDim2.new(1 / 3, -20, 0, 34),
			BackgroundColor3 = state.crosshairShape == shape and state.accent or theme.card2,
			AutoButtonColor = false,
			Text = string.upper(shape),
			TextColor3 = state.crosshairShape == shape and Color3.fromRGB(8, 8, 11) or theme.text,
			Font = Enum.Font.GothamSemibold,
			TextSize = 12,
			Parent = crosshairShapeRow
		})
		...
		button.MouseButton1Click:Connect(function()
			state.crosshairShape = shape
			drawCrosshair()
			renderSettings()
		end)
	end

	local crosshairColorRow = create("Frame", {
		Size = UDim2.new(1, -14, 0, 92),
		BackgroundColor3 = theme.card,
		BorderSizePixel = 0,
		Parent = contentFrame
	...
	create("TextLabel", {
		...
		Font = Enum.Font.GothamSemibold,
		TextSize = 14,
		Parent = crosshairColorRow
	})

	crosshairColorPreview = create("Frame", {
		Position = UDim2.fromOffset(14, 42),
		Size = UDim2.fromOffset(34, 34),
		BackgroundColor3 = state.crosshairColor,
		BorderSizePixel = 0,
		Parent = crosshairColorRow
	})
	...
	savePassword.MouseButton1Click:Connect(function()
		...
		if nextPassword == "" then
			newPasswordInput.PlaceholderText = "Password cannot be empty"
			return
		end
	...
	saveBranding.MouseButton1Click:Connect(function()
		...
		if nextLogo == "" or string.lower(nextLogo) == "sparkle" then
			state.logoMode = "sparkle"
			state.logoText = ""
			logoInput.Text = "sparkle"
		else
			state.logoMode = "text"
			state.logoText = string.sub(nextLogo, 1, 3)
		end
	...
	savePanel.MouseButton1Click:Connect(function()
		...
		if state.authenticated and not state.minimized then
			tween(panel, 0.2, { Size = openPanelSize() })
		end
	end)

	watermarkToggle.MouseButton1Click:Connect(function()
		state.watermarkEnabled = not state.watermarkEnabled
		applyWatermark()
	end)

	saveWatermark.MouseButton1Click:Connect(function()
		local nextText = trim(watermarkInput.Text)
		state.watermarkText = nextText ~= "" and nextText or "onyx"
		applyWatermark()
	end)

	resetWatermark.MouseButton1Click:Connect(function()
		state.watermarkPosition = UDim2.new(1, -276, 0, 12)
		state.watermarkSize = Vector2.new(260, 30)
		applyWatermark()
	end)
...
local function submitPassword()
	...
	if typed == "" then
		setStatus("error", "Access denied", "Enter the password first.")
		return
	end
...
local function buildHeader()
	...
	local function headerButton(name, text, rightOffset)
		local button = create("TextButton", {
			...
			Position = UDim2.new(1, rightOffset, 0, 14),
	...
	minimizeButton.MouseButton1Click:Connect(function()
		...
		if state.minimized then
			...
			body.Visible = true
			minimizeButton.Text = "-"
			tween(panel, 0.2, { Size = state.authenticated and openPanelSize() or authPanelSize() })
		end
	end)
...
local function buildAuth()
	...
	submitButton = create("TextButton", {
		...
		AutoButtonColor = false,
		Text = "SUBMIT",
		TextColor3 = Color3.fromRGB(8, 8, 11),
		Font = Enum.Font.GothamSemibold,
		TextSize = 14,
		Parent = authView
	})
...
local function buildGui()
	...
	watermarkLabel.InputBegan:Connect(function(input)
		...
		if state.watermarkResizing then
			return
		end
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
	...
	panelScale = create("UIScale", {
		Scale = 1,
		Parent = panel
	})
	create("UISizeConstraint", {
		MinSize = Vector2.new(320, 58),
		MaxSize = Vector2.new(780, 620),
		Parent = panel
	})
...
UserInputService.InputChanged:Connect(function(input)
	...
	elseif state.watermarkDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
		...
		state.watermarkPosition = UDim2.new(
			state.watermarkStartPosition.X.Scale,
			state.watermarkStartPosition.X.Offset + delta.X,
			state.watermarkStartPosition.Y.Scale,
			state.watermarkStartPosition.Y.Offset + delta.Y
		)
		applyWatermark()
	elseif state.watermarkResizing and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
		local delta = input.Position - state.watermarkDragStart
		state.watermarkSize = Vector2.new(
			math.clamp(state.watermarkStartSize.X + delta.X, 80, 700),
			math.clamp(state.watermarkStartSize.Y + delta.Y, 20, 140)
		)
		applyWatermark()
	end
end)
...
UserInputService.InputBegan:Connect(function(input, processed)
	...
	elseif input.KeyCode == Enum.KeyCode.Tab and screenGui.Enabled then
		if passwordInput and not state.authenticated then
